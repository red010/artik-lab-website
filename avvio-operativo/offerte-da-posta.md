# Offerte e listini da posta e file

Arriva una richiesta in mail, spesso con PDF sparsi. A fine giornata resta una bozza di offerta, sul modello dell'azienda, da far rileggere a chi vende.

- Durata: Un giorno in sede

## Il problema

Serve un preventivo o un listino aggiornato, con le schede allegate. Oggi chi vende perde tempo a copiare da tre cartelle. Trattare, chiudere e inserire l'ordine restano alla persona. Si alleggerisce il lavoro di raccolta: mettere insieme i pezzi e segnalare cosa manca, prima che chi vende tratti.

## Da dove si prende e cosa si produce

Mail, PDF di richiesta, listini interni, schede prodotto, modello di offerta dell'azienda. Non si scrive nel gestionale. Non si entra in PriMus e non si tocca il disegno.

## Cosa succede in giornata

Si scrive come si fa, passo passo: da dove si prendono listino e schede, come si compone la bozza, dove si ferma la persona (prezzo, sconto, sì o no). Si prova prima su tre richieste d'esempio disordinate, poi sui file veri. A fine giornata la procedura è scritta e la settimana dopo qualcuno in azienda la sa rifare.

## Cosa si lascia

- Le istruzioni, in italiano.
- Come si organizzano le cartelle: in arrivo, bozze, da rileggere, archivio.
- Cosa controllare: campi obbligatori, allegati attesi, cosa segnalare se manca un pezzo.
- Dove si ferma la persona: prezzo, sconto, invio.

## Com'è la giornata

1. **Si parte dagli esempi**: Si parte da un ambiente già pronto, con tre richieste fittizie, prima di toccare i file veri.
2. **Si prova su allegati incompleti o sbagliati**: Un allegato storto, un codice che non esiste, la stessa ragione sociale scritta in due modi: si vede come viene trattato.
3. **Si passa ai file veri**: Gli esempi restano da una parte. I file dell'azienda li sostituiscono, non li cancellano.
4. **Si produce una bozza vera**: Almeno un'offerta o un listino sul modello dell'azienda, da rileggere.
5. **Cosa resta**: Un responsabile interno, un tempo da misurare concordato prima, la procedura scritta.

## Esempi su cui si prova

Inventati, disordinati, mai da clienti.

### Mail di Nord Latticini Srl (azienda inventata)

Oggetto: «urgente listino HORECA??». Corpo con ragione sociale scritta in due modi, un PDF ruotato, una scheda prodotto in .docx con allergeni in una tabella spezzata, un listino Excel con una colonna prezzi vuota.

### PEC di Officina Valle Bruna (azienda inventata)

Richiesta d'offerta su un particolare tornito, disegno in PDF a bassa risoluzione, quantità «circa 200 / da confermare», scadenza a ieri, tre codici articolo di cui uno non esiste nel listino d'esempio.

### Mail di Edilcantiere Quarto (azienda inventata)

Capitolato copiato nel corpo della mail, prezzi fornitori in tre PDF con unità di misura diverse, una voce «come da ultimo», un allegato mancante («vi mando dopo la scheda del massetto»).

## Cosa resta in azienda

La procedura scritta, l'intelligenza artificiale già impostata sull'account dell'azienda, i punti in cui si deve fermare una persona, almeno una bozza vera da rileggere, un responsabile interno. La settimana dopo si rifà senza Artik Lab presente.

## Quando non è adatto

- Il «preventivo» è un computo da disegno (PriMus, AutoCAD) o una stima di ore di cantiere: quello è il mestiere, non questo servizio.
- Serve trattare, chiudere lo sconto, o decidere il prezzo in gara.
- Serve scrivere l'ordine o la conferma nel gestionale.
- La richiesta arriva una o due volte all'anno.
- Listino e schede non esistono, o esistono solo nella testa di una persona e non si possono copiare in una cartella.
- Si chiede che l'offerta parta da sola, senza rilettura.

## Cosa serve prima.

- Un abbonamento all'intelligenza artificiale sull'account dell'azienda.
- Una persona interna che già mette insieme le offerte, con l'incarico di usarla dopo.
- Un modello di offerta o di listino, anche imperfetto.
- Una cartella o un file da cui prendere listini e schede, senza entrare nel gestionale.

Si lavora sui file dell'azienda, sull'intelligenza artificiale già impostata sull'account dell'azienda. Artik Lab non porta via i documenti. Sono escluse le integrazioni su misura con i software già in uso. Gli esempi sono inventati.

## Il settore è solo un'etichetta; il lavoro è lo stesso

Il settore è solo un'etichetta. Il lavoro è lo stesso.

### Trasformazione alimentare

Listino per GDO o HORECA e schede prodotto da mail.

### Meccanica terzista

Richiesta d'offerta con disegno in PDF.

### Edilizia o impianti

Capitolato e prezzi fornitori da mettere insieme prima del computo metrico, che resta fuori.

## Domande frequenti

### L'offerta parte da sola?

No. Esce una bozza. Chi vende rilegge, decide prezzo e sconto, invia.

### Si entra in PriMus?

No. Se il preventivo è un computo da disegno, questo servizio non è adatto.

### Si aggiorna il gestionale?

No.

### Serve un commerciale in aula?

Serve chi già mette insieme le offerte, con l'incarico di continuare. Non è un corso per la rete vendita.


## Servizi collegati

- [Avvio operativo](https://ar-tik.com/avvio-operativo/index.md)
